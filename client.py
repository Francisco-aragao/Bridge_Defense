...
class GameState:
    ...
    def initCannons(self, cans: list[list[int]]):
        self.cannons = [(l[0], l[1]) for l in cans]

    def getPossibleCannonTargets(
        self, cannon: tuple[int, int]
    ) -> list[tuple[int, dict[str, Any]]]:
        """
        Check possible cannon targets.

        This function checks the current board and identifies all ships currently in range of `cannon`.

        Parameters
        ----------
        `cannon`: Cannon coordinates as a tuple (bridge, river).

        Returns
        -------
        `targets`: A list of possible targets each as a tuple (river, ship).
        """

        bridge: int = cannon[0]
        river: int = cannon[1]
        targets: list[tuple[int, dict[str, Any]]] = list()

        # Target at river coordinate
        if river != 0:
            # If there are ships at (bridge, river), add them to possible targets
            if len(self.board[bridge][river]) > 0:
                for ship in self.board[bridge][river]:
                    targets.append((river, ship))

        # Target below river coordinate
        if river != NUM_RIVERS:
            # If there are ships at (bridge, river + 1), add them to possible targets
            if len(self.board[bridge][river + 1]) > 0:
                for ship in self.board[bridge][river + 1]:
                    targets.append((river + 1, ship))

        return targets

    def initFirstBridge(self, stateFirstBridge: list[dict[str, Any]]):
        """
        Initialize all ships in the first bridge.

        This function is meant to be used to update the state of the first bridge at every turn.
        ...
        for river in range(1, NUM_RIVERS + 1):
            self.board[1][river] = cp.deepcopy(stateFirstBridge[river - 1]["ships"])

    def advanceTurn(self, stateFirstBridge: list[dict[str, Any]]):
        """
        Advance game to the next turn:
            - Last bridge ships are cleared.
            - Every ship not sunk advances to the next bridge.
            - First bridge is updated with incoming ships.
        ...
        # Last bridge ships are gone :(
        for river in range(1, NUM_RIVERS + 1):
            self.board[NUM_BRIDGES][river].clear()

        # Move remaining ships to next bridge
        for bridge in range(NUM_BRIDGES, 1, -1):
            self.board[bridge] = cp.deepcopy(self.board[bridge - 1])

        # Add new ships
        self.initFirstBridge(stateFirstBridge)
        self.turn += 1

    def shootCannon(self, cannon: tuple[int, int], targetRiver: int, id: int):
        """
        Shoot `cannon` at ship with `id` located at `targetRiver`.

        If the ship will sink, it is removed from the list of ships at its location.\n
        If the ship will survive, its `hits` counter is incremented.

        Parameters
        ----------
        `cannon`: Cannon coordinates as a tuple (bridge, river).
        `targetRiver`: River where the ship is currently located.
        `id`: Ship id.
        """

        bridge: int = cannon[0]
        ships: list[dict[str, Any]] = self.board[bridge][targetRiver]

        # Search for ship at position (bridge, targetRiver)
        for s in range(len(ships)):
            ship: dict[str, Any] = ships[s]

            if ship["id"] == id:
                # Check if this ship will sink
                # Update hits or destroy ship

                if SHIP_HP[ship["hull"]] - ship["hits"] == 1:
                    self.board[bridge][targetRiver].remove(ship)
                else:
                    self.board[bridge][targetRiver][s]["hits"] += 1
